...
import { WATCHES, getWatch, relatedWatches } from "@/data/watches";
...
export async function generateMetadata({
  ...
  if (!w) return { title: "Не найдено" };
  return {
    title: `${w.brand} ${w.model}`,
    description: `${w.brand} ${w.model}, ${w.ref}, ${w.year}. ${w.description.slice(0, 140)}`,
  };
}
...
export default async function WatchPage({ params }: { params: Promise<Params> }) {
  ...
  const brand = getBrand(w.brandSlug);
  const related = relatedWatches(w.slug, 4);
    ...
    <>
      ...
      <main className="flex-1 pt-16 md:pt-20">
        <section className="relative overflow-hidden pb-12 md:pb-20 bg-paper border-b border-line">
          ...
          <Fog x="75%" y="55%" size={500} color="#0F5A40" opacity={0.22} blur={100} />
          ...
          <div className="mx-auto max-w-[110rem] px-6 md:px-10">
            <Breadcrumb
              items={[
                ...
                { href: `/brands/${w.brandSlug}`, label: w.brand },
                { label: w.model },
              ]}
            />
            ...
            <div className="mt-7 md:mt-10 grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 lg:gap-16 items-start">
              ...
              <div className="lg:col-span-6">
                ...
                >
                  {w.model}<span style={{ color: "var(--sapphire)" }}>.</span>
                </h1>

                <div className="mt-5 flex flex-wrap gap-2">
                  <span className="tag">{w.tag}</span>
                  <span className="tag tag--ghost">REF · {w.ref}</span>
                  <span className="tag tag--ghost">{w.year}</span>
                  <span className="tag tag--ghost">{w.condition}</span>
                  {w.papers && <span className="tag tag--ghost">ПАСПОРТ</span>}
                  {w.box && <span className="tag tag--ghost">КОРОБКА</span>}
                </div>
          ...
          <section className="relative py-14 md:py-20 overflow-hidden">
            ...
            <div className="mx-auto max-w-[110rem] px-6 md:px-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {related.map((p, i) => <ProductCard key={p.slug} p={p} i={i} />)}
              </div>
            </div>
          </section>
        )}
