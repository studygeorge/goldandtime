...
import type { Watch, WatchTagVariant } from "@/data/watches";

function tagClass(v: WatchTagVariant) {
...
export function ProductCard({ p, i = 0 }: { p: Watch; i?: number }) {
  ...
  return (
    ...
    >
      <Link href={`/watches/${p.slug}`} className="block">
        <div className="relative h-[200px] sm:h-[230px] bg-white overflow-hidden">
          {hero ? (
            <>
              <Image
                ...
                className={`object-contain p-4 transition-opacity duration-500 ${second ? "group-hover:opacity-0" : "transition-transform group-hover:scale-[1.04]"}`}
                priority={i < 4}
              />
              {second && (
                <Image
          ...
          <div className="absolute bottom-3 left-3 flex gap-1.5 flex-wrap z-10">
            {p.papers && <span className="t-mono-cap bg-white/80 backdrop-blur px-2 py-1 text-ink">ПАСПОРТ</span>}
      ...
      <div className="p-4 sm:p-5 border-t border-line flex-1 flex flex-col">
        <div className="t-mono-cap text-ink-3 mb-1.5">{p.brand}</div>
        <Link href={`/watches/${p.slug}`} className="block">
